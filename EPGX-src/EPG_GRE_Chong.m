function [F0,Fn,Zn,F] = EPG_GRE_Chong(theta,phi,TR,T1,T2,varargin)
...
%   Add zinit argument for MOLLI which chains multiple bSSFP readout
...
%     % Check if there is MT-prep pulse before bSSFP
%     % Chong Duan, 2018-01-29
%     if isfield(prep, 'prepMT') && prep.prepMT
%         disp(FF(3))
%         FF(3) = FF(3)*0.8;       % assume a constant MTR
%         disp(FF(3))
%     end
end
