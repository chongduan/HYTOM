function [F0,Fn,Zn,F] = EPGX_GRE_MT_HYTOM(theta,phi,B1SqrdTau,TR,T1x,T2a,f,ka,G,varargin)
...
%   Included zinit argument for MOLLI which chains multiple bSSFP readouts
%   Chong Duan 2018-01-22
...
if exist('prep','var')
    ...
    % Add the MT effect for the prep pulse
    % Chong Duan 2018-01-24
    ...
    % Add the MTC-prep pulse
    if isfield(prep, 'MTC') && prep.MTC
        z0 = FF([3 4]);
        [ zout ] = MTC_prep(z0, T1x, T2a, f, ka, G * 1e3); % G * 1e3 --> convert back to us
        FF([3 4]) = zout;
    end
end
